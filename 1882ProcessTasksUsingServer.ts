...
function assignTasks(servers: number[], tasks: number[]): number[] {
  const ans: number[] = [];
  const serverInfoArr: ServerInfo[] = [];
  ...
  while (taskIdx < tasks.length) {
    ...
    while (taskQueue.length > 0) {
      console.log("here");
      const nextServerIdx: number = getNextServer(serverInfoArr);
      if (nextServerIdx > -1) {
    ...
    for (let j = 0; j < serverInfoArr.length; j++) {
...
function getNextServer(serverInfoArr: ServerInfo[]) {
  let idx = -1;
  for (let i = 0; i < serverInfoArr.length; i++) {
