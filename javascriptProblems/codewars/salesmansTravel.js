...
const travel = (r, zipcode) => {
  ...
  for (const entry of list) {
    if (entry.slice(-8) === zipcode) {
      const houseNum = entry.split(' ').splice(0, 1).join('')
      houseNumbers.push(houseNum)

      const street = entry.slice(houseNum.length, -8).trim()
      streets.push(street)
    }
  }
...
console.log(travel(ad, "NY 56432"))
console.log(travel(ad, "NY 5643"))