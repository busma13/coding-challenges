...
var successfulPairs = function(spells, potions, success) {
    ...
    for (let i = 0; i < spells.length; i++) {
        ...
        let idx = Math.floor(potions.length/2);
        let idxOld = -Infinity;
        console.log(i, idx, idxOld)
        while (Math.abs(idx - idxOld) > 1) {
            if (spells[i] * potions[idx] < success) {
                idxOld = idx;
                idx = Math.floor((potions.length + idx) / 2);
                console.log('less', spells[i] * potions[idx])
            } else {
                idxOld = idx;
                idx = Math.floor((potions.length - idx) / 2);
                console.log('greater', spells[i] * potions[idx])
            }
            console.log('loop', i, idx, idxOld)
        }
        if (idx > idxOld) {
            pairs.push(potions.length - idx);
        } else {
            pairs.push(potions.length - idxOld);
        }
